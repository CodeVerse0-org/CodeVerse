  ...
  return (
    <nav className="bg-[#0D1117] border-b border-[#21262D] px-6 py-4 flex justify-between items-center fixed top-0 left-0 right-0 z-50">
      <div className="flex items-center gap-2">
        <div className="w-8 h-8 bg-gradient-to-tr from-[#3FB950] to-[#238636] rounded-lg flex items-center justify-center font-bold text-white shadow-md shadow-[#238636]/20">
          C
        </div>
      ...
      <div className="flex items-center gap-6">
        <div className="relative">
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-[#3FB950] text-black font-bold text-[10px] w-5 h-5 rounded-full flex items-center justify-center animate-pulse border-2 border-[#0D1117]">
                {unreadCount}
              </span>
            )}
          </button>
          ...
          {dropdownOpen && (
            <div className="absolute right-0 mt-3 w-80 bg-[#161B22] border border-[#21262D] rounded-xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="p-4 border-b border-[#21262D] flex justify-between items-center bg-[#0D1117]">
                <h3 className="font-semibold text-white">Notifications</h3>
                {unreadCount > 0 && (
                  <button
                    onClick={markAllAsRead}
                    className="text-xs text-[#3FB950] hover:underline font-medium"
                  >
                    Mark all read
                  </button>
                )}

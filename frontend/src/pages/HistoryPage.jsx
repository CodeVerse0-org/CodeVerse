...
import { Clock, Database, ChevronRight, Layout, Search, Calendar } from "lucide-react";
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
            ...
            <div className="relative group w-full md:w-80">
              ...
              <input 
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-12 pr-4 text-xs font-bold text-white focus:outline-none focus:border-cyan-500/50 transition-all"
              />
            </div>
          </div>
            ...
            {loading ? (
              ...
              filteredHistory.map((item) => (
                <div 
                  ...
                  className="group relative flex items-center justify-between p-5 bg-[#0a0c0f] border border-white/5 rounded-2xl hover:border-cyan-500/40 transition-all cursor-pointer overflow-hidden"
                >
                  <div className="flex items-center gap-6 relative z-10">
                    <div className="w-14 h-14 flex items-center justify-center bg-black rounded-xl border border-white/10 group-hover:border-cyan-500/30 transition-all">
                      <Database size={22} className="text-gray-500 group-hover:text-cyan-400" />
                    </div>
                    <div>
                      <h3 className="text-white font-black text-sm uppercase tracking-wider group-hover:text-cyan-300">
                        {item.repo_name}
                      </h3>
                      <div className="flex items-center gap-4 mt-2">
                        <span className={`flex items-center gap-1.5 text-[9px] font-black px-2 py-0.5 rounded-md border uppercase ${
                          item.graph_type === 'file' ? 'bg-cyan-500/5 border-cyan-500/20 text-cyan-500' : 'bg-rose-500/5 border-rose-500/20 text-rose-500'
                        }`}>
                  ...
                  <ChevronRight className="text-gray-700 group-hover:text-cyan-400 transition-all" size={20} />
                </div>
              ))
            ) : (
              <div className="py-32 flex flex-col items-center justify-center border border-dashed border-white/10 rounded-3xl bg-white/[0.01]">
                ...
                <p className="text-gray-500 font-bold uppercase tracking-widest text-xs">No records found</p>
              </div>
            )}
          </div>
        </main>

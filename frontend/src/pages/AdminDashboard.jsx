...
import { Bell, RefreshCw, Github } from "lucide-react";
    ...
    <div className="min-h-screen flex bg-gradient-to-br from-black via-[#041a1f] to-black text-white font-sans">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <div className="flex justify-between items-center mb-10">
          <h1 className="text-2xl font-bold tracking-tight text-cyan-50">System Overview</h1>
          <div className="flex items-center gap-4">
            <div className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] font-bold uppercase tracking-widest flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${isGithubConnected ? "bg-green-500 shadow-[0_0_8px_#22c55e]" : "bg-red-500 shadow-[0_0_8px_#ef4444]"}`} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-black/40 border border-white/10 p-6 rounded-2xl backdrop-blur-md">
            <p className="text-gray-400 text-xs uppercase tracking-wider mb-2 font-semibold">Connection Status</p>
            <p className={`text-2xl font-black ${isGithubConnected ? "text-green-400" : "text-red-500"}`}>
              {isGithubConnected ? "ONLINE" : "OFFLINE"}
            </p>
            {!isGithubConnected && (
              <button onClick={handleConnectGitHub} className="mt-3 flex items-center gap-2 text-[10px] font-bold text-cyan-400 hover:text-white transition-colors">
        ...
        <div className="mt-10 p-20 border border-dashed border-white/5 rounded-3xl text-center">
            <p className="text-gray-600">Module specific reporting tools will appear here.</p>
        </div>
      </main>
    </div>
  );
